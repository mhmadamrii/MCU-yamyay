  ...
  return (
    <header className="sticky top-0 z-40 border-b border-gutter bg-ink/95 backdrop-blur-[2px]">
      <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-2.5 sm:px-6">
        <a
          href="#top"
          className="cover-type text-lg leading-none text-paper focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-plate-red-bright sm:text-xl"
        >
          ...
          <ul className="flex items-center gap-1">
            {NAV.map((entry) => (
              <li key={entry.href}>
                <a
                  href={entry.href}
                  className="credit-type block px-2.5 py-1 text-[0.6rem] text-paper-dim transition-colors hover:bg-panel-raised hover:text-paper focus-visible:outline-2 focus-visible:outline-plate-red-bright"
                >
                  {entry.label}
                </a>
              </li>
        ...
        <p className="credit-type ml-auto shrink-0 text-[0.6rem] text-paper-dim">
        ...
        <ul className="flex">
          {NAV.map((entry) => (
            <li key={entry.href} className="flex-1">
              <a
                href={entry.href}
                className="credit-type block py-2 text-center text-[0.6rem] text-paper-dim"
              >
                {entry.label}
              </a>
            </li>
